...
import { gitClone, getLastCommit, getCommitInfo, CommitInfo } from '../child_process/index'

import { SettingsResponse, Settings } from './routes'

const router = express.Router()

function isValidConfigarationSettings(settings: Settings): boolean {
...
router.get('/', async (req, res, next) => {
	...
	yndxDbApi.get<{}, AxiosResponse<SettingsResponse>>('/conf').then(r => {
		if (r.status === 200) {
...
// POST /api/settings
router.post<{}, { message: string } | string>('/', (req, res, next) => {
	const settings = req.body

	let tmpCommitHash: null | CommitInfo['commitHash'] = null
	...
	gitClone(settings)
		...
		.then(commitInfo => {
			...
			return yndxDbApi.post('/build/request', commitInfo)
		})
		.then(() =>
...
router.delete('/', (req, res) => {
	yndxDbApi.delete('/conf').then(() => res.send('Конфигурация удалена.'))
})
